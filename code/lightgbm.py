...
# tarin module
for train_idx, test_idx, in StratifiedKFold(n_splits=7).split(train_X, train_df['interest_level']):
    clf.fit(train_X[train_idx], train_y[train_idx], eval_set=[(train_X[test_idx], train_y[test_idx]), (train_X[test_idx], train_y[test_idx])], verbose=50, early_stopping_rounds=10)
    loss = log_loss(train_y[test_idx], clf.predict_proba(train_X[test_idx]))
    loss_score.append(loss)
    print('Val loss:', min(loss_score))